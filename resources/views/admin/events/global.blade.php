@extends('layouts.admin', ['title' => 'Kelola Event Global'])

@section('content')
<header class="flex justify-between items-center mb-10">
    <div>
        <h1 class="text-3xl font-black">Moderasi Event Global</h1>
        <p class="text-slate-500 font-medium">Pantau, ubah, atau take down event dari seluruh vendor.</p>
    </div>
    <!-- Tombol tambah dihapus karena Superadmin tidak membuat event sendiri -->
</header>
...
<div class="mb-6">
    <form action="{{ route('admin.events.index') }}" method="GET" class="flex gap-3 max-w-md">
        <div class="relative flex-1">
            ...
            <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari judul event global..." class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
        </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-slate-50 text-slate-400 uppercase text-[10px] font-black tracking-widest">
                <tr>
                    ...
                    <th class="px-8 py-4">Vendor Pemilik</th> <!-- 🌟 Kolom Asal Vendor -->
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <!-- 🌟 Info Vendor Pemilik Event -->
                        <span class="px-3 py-1 bg-indigo-50 text-indigo-600 font-black text-xs rounded-xl uppercase tracking-wider">
                            {{ $event->user->name ?? 'Unknown Vendor' }}
                        </span>
                    </td>
                    <td class="px-8 py-6">
                    ...
                    <td class="px-8 py-6 flex gap-2">
                        ...
                        <form action="{{ route('admin.events.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Hapus paksa event ini dari sistem?')">
                            @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="6" class="px-8 py-12 text-center text-slate-400 font-medium">
                        Belum ada event terdaftar lintas vendor.
                    </td>
                </tr>
                @endforelse
            </tbody>
